...
void TrafficLight::cycleThroughPhases()
{
    ...
    int cycleCounts = distrib(gen);
    auto lastLoopTime = std::chrono::system_clock::now();
    while(true){
        auto durSeconds = std::chrono::duration_cast<std::chrono::milliseconds>
                (std::chrono::system_clock::now() - lastLoopTime);
        int durints = durSeconds.count();

        if(durints >= cycleCounts){
